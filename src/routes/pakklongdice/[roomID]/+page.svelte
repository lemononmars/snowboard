...
<div id="game-area">
  <Group>
    ...
    {#if $stateIndex === GAME_STATUS_PREGAME || $stateIndex == GAME_STATUS_GAMEEND}
      <Button on:click={() => returnToLobby()}
        ><Label>Return to lobby</Label></Button
      >
    {/if}
  </Group>
  ...
  {#if $stateIndex === 1 || $stateIndex === 2}
    <div class="timercontainer">
      <div class="progress_container" style="display:block">
        <div
          class="progress_bar"
          style="width:{percentage_str}; background-color:{timer.color}"
        >
          &nbsp;
        </div>
      </div>
        ...
        <tr>
          {#each actionButtonImgUrl as url, i}
            <td>
              <button
                class="action-button"
                style="background-color: {actionButtonColors[actionStatus[i]]}"
                value={i}
                on:click={() => submitAction(i)}
              >
...
<div id="score-area">
  {#if $stateIndex != 0}
    <span
      >Round {$gameInfo.roundInfo.round} / {$gameInfo.gameConfigs
        .gameLength}</span
    ><br />
    <span
      >Difficulty: {difficulty_str}, Shuffle: {$gameInfo.gameConfigs
        .shuffle}</span
    >
  {/if}
